...
class ModifiedTimeFeatureWrapper(gym.Wrapper[TimeFeatureObs, ActType, TimeFeatureObs, ActType]):
    """
    ...
    def __init__(self, env: gym.Env, max_steps: int = 1000, test_mode: bool = False, test_mode_steps_horizon: int = 0):
        assert isinstance(
    ...
    def _get_obs(self, obs: Union[np.ndarray, Dict[str, np.ndarray]]) -> Union[np.ndarray, Dict[str, np.ndarray]]:
        ...
        if self._test_mode:
            time_feature = 1 - (self._test_mode_steps_horizon / self._max_steps)
