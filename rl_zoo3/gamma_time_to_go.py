...
class GammaTimeFeatureWrapper(gym.Wrapper[TimeFeatureObs, ActType, TimeFeatureObs, ActType]):
    """
    ...
    def __init__(self, env: gym.Env, max_steps: int = 1000, test_mode: bool = False, test_mode_steps_horizon = 0, num_gammas: int = 10):
        assert isinstance(
        ...
        low, high = np.concatenate((low, [0.0] * num_gammas)), np.concatenate((high, [1.0] * num_gammas))  # type: ignore[arg-type]
        self.gamma_range = np.linspace(0.99, 1.0, num_gammas)
        ...
        self._test_mode_steps_horizon = test_mode_steps_horizon
    ...
    def _get_obs(self, obs: Union[np.ndarray, Dict[str, np.ndarray]]) -> Union[np.ndarray, Dict[str, np.ndarray]]:
        ...
        time_feature = self.gamma_range ** (self._max_steps - self._current_step)
        if self._test_mode:
            time_feature = self.gamma_range ** self._test_mode_steps_horizon
